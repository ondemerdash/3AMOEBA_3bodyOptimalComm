...
c     molecular dynamics routine, dynamic.f
      program analyze_parallel_bcast_evenload_bsend_noreduce
      implicit none
c      include 'sizes.i'
      include 'sizes3b.i'
c      include 'atoms.i'
...
c      include 'inter.i'
      ...
      logical doanalysis
      integer i1,i3,ii,jj,k,moli2,moli3,count3,iter3
      real*8 energy,step
      real*8 cutoff
      ...
      real*8 ep3bt_tot2_recv,ep3bt_tot3_recv
      real*8 virep3bt_tot2_recv(3,3),virep3bt_tot3_recv(3,3)
      ...
      real*8, allocatable :: dep3bt_tot2_recv(:,:)
      real*8, allocatable :: dep3bt_tot3_recv(:,:)
      integer offset,remainder,start
      integer localsum,totalsum,localsum_tot
      integer stat(MPI_STATUS_SIZE)
c      integer,allocatable :: mol3new(:)
      character*6 mode
      integer s1,s2,s3,s4,totsize1,totsize2,totsize3b
      integer, allocatable :: buf1(:)
      integer, allocatable :: buf2(:)
c      real*8, allocatable :: buf2b(:)
      real*8, allocatable :: buf3b(:)
...
c     totals of these, dep3bt_tot2 and dep3bt_tot3.  
      ...
      allocate (dep3bt_tot2_recv(3,npole))
      allocate (dep3bt_tot3_recv(3,npole))
      end if
...
c  Build 2- and 3-body neighbor lists
      ...
      call mpi_pack_size(3*npole+3*3+1,mpi_real8,mpi_comm_world,
     &     s1,ierr)
c      print*,"Before pack size2"
      call mpi_pack_size(3*npole+3*3+1,mpi_real8,mpi_comm_world,
     &     s2,ierr)

      totsize3b=numtasks*(s1+s2+2*mpi_bsend_overhead)
      allocate (buf3b(totsize3b))
      print*,"totsize3b",totsize3b

c      print*,"Before pack size1"
      call mpi_pack_size(800*offset,mpi_integer,mpi_comm_world,
     &     s1,ierr)
c      print*,"Before pack size2"
      call mpi_pack_size(offset,mpi_integer,mpi_comm_world,
     &     s2,ierr)

      totsize1=numtasks*(s1+s2+2*mpi_bsend_overhead)
     ...
     &     s4,ierr)
      ...
      if(taskid.eq.master) then
        ...
        if(domollst3bod) call molfull3bodycobar
           do i=1,numtasks
           call mpi_bsend(mollst3mod(1:800,((i-1)*offset+1):i*offset),
     &      800*offset,mpi_integer,i-1,2*(i-1),mpi_comm_world,ierr)
           call mpi_bsend(nmollst3mod(((i-1)*offset+1):i*offset),offset,
     &      mpi_integer,i-1,2*(i-1)+1,mpi_comm_world,ierr)
           end do
           print*,"After 1st send"
           do i=1,remainder
             call mpi_bsend(mollst3mod(1:800,
     &      (numtasks*offset+i):(numtasks*offset+i)),800,mpi_integer,
     &      i-1,2*numtasks+2*(i-1),mpi_comm_world,ierr)
             call mpi_bsend(nmollst3mod((numtasks*offset+i):
     &        (numtasks*offset+i)),1,mpi_integer,i-1,
     &      2*numtasks+2*(i-1)+1,mpi_comm_world,ierr)
           end do
          print*,"After 2nd send"
      end if


       allocate (mollst3mod_recv(800,offset))
       allocate (nmollst3mod_recv(offset))
       allocate (mollst3mod_recv_small(800,1))
       allocate (nmollst3mod_recv_small(1))
          call mpi_recv(mollst3mod_recv,800*offset,mpi_integer,master,
     &      2*taskid,mpi_comm_world,stat,ierr)
          call mpi_recv(nmollst3mod_recv,offset,mpi_integer,master,
     &      2*taskid+1,mpi_comm_world,stat,ierr)

         if(taskid.le.remainder-1) then
          call mpi_recv(mollst3mod_recv_small,800,mpi_integer,master,
     &      2*numtasks+2*(taskid),mpi_comm_world,stat,ierr)
          call mpi_recv(nmollst3mod_recv_small,1,mpi_integer,master,
     &      2*numtasks+2*(taskid)+1,mpi_comm_world,stat,ierr)
         end if
...
c
        call mpi_buffer_detach(buf1,totsize1,ierr)
        call mpi_buffer_detach(buf2,totsize2,ierr)
...
c      call mpi_pack_size(3*npole+3*3+1,mpi_real8,mpi_comm_world,
c     &     s1,ierr)
c      print*,"Before pack size2"
c      call mpi_pack_size(3*npole+3*3+1,mpi_real8,mpi_comm_world,
c     &     s2,ierr)
...
c      allocate (buf3b(totsize1))

c      print*,"Before 1st buffer attach"
      if(i3.eq.10) then
         print*,"Before buf3b attach"
      end if
      call mpi_buffer_attach(buf3b,totsize3b,ierr)
c      print*,"After 1st buffer attach"
      if(i3.eq.10) then
         print*,"After buf3b attach"
      end if
...
c                  call mpi_reduce(ep3bt_tot2,ep3b2,1,mpi_real8,
c     &          mpi_sum,master,mpi_comm_world,ierr)
c                  call mpi_reduce(dep3bt_tot2,dep3b2,npole*3,mpi_real8,
c     &          mpi_sum,master,mpi_comm_world,ierr)
c                  call mpi_reduce(virep3bt_tot2,virep3b2,3*3,mpi_real8,
c     &          mpi_sum,master,mpi_comm_world,ierr)
                  call mpi_bsend(ep3bt_tot2,1,mpi_real8,master,
     &            3*taskid+1,mpi_comm_world,ierr)
                  call mpi_bsend(dep3bt_tot2,3*npole,mpi_real8,master,
     &            3*taskid+2,mpi_comm_world,ierr)
                  call mpi_bsend(virep3bt_tot2,3*3,mpi_real8,master,
     &            3*taskid+3,mpi_comm_world,ierr)
...
c                 moli1=mol3new(k1)
                 iter3=iter3+1
                 call Innerloop3(iter3,moli1,ep3bt,virep3bt,dep3bt)
                 do i=1,3
                   do j=1,3
...
c                  call mpi_reduce(ep3bt_tot3,ep3b3,1,mpi_real8,
c     &          mpi_sum,master,mpi_comm_world,ierr)
c                  call mpi_reduce(dep3bt_tot3,dep3b3,npole*3,mpi_real8,
c     &          mpi_sum,master,mpi_comm_world,ierr)
c                  call mpi_reduce(virep3bt_tot3,virep3b3,3*3,mpi_real8,
c     &          mpi_sum,master,mpi_comm_world,ierr)
                  call mpi_bsend(ep3bt_tot3,1,mpi_real8,master,
     &            3*numtasks+3*taskid+1,mpi_comm_world,ierr)
                  call mpi_bsend(dep3bt_tot3,3*npole,mpi_real8,master,
     &            3*numtasks+3*taskid+2,mpi_comm_world,ierr)
                  call mpi_bsend(virep3bt_tot3,3*3,mpi_real8,master,
     &            3*numtasks+3*taskid+3,mpi_comm_world,ierr)
             else
               start=taskid*offset+1
               iter3=0
               do moli1 =start,start+offset-1
...
c    other noncovalent components, as well as the covalent energies.
           if(taskid.eq.master) then
             do i=0,numtasks-1
                if(i3.eq.10) then
                  print*,"Beginning of loop In final recv iter 10",i
                end if
                call mpi_recv(ep3bt_tot2_recv,1,mpi_real8,i,
     &          3*i+1,mpi_comm_world,stat,ierr)
                ep3b2=ep3b2+ep3bt_tot2_recv
  
               call mpi_recv(dep3bt_tot2_recv,3*npole,mpi_real8,i,
     &          3*i+2,mpi_comm_world,stat,ierr)
                do ii=1,npole 
                   do jj=1,3
                     dep3b2(jj,ii)=dep3b2(jj,ii)+dep3bt_tot2_recv(jj,ii)
                   end do
                end do
 
               call mpi_recv(virep3bt_tot2_recv,3*3,mpi_real8,i,
     &          3*i+3,mpi_comm_world,stat,ierr)
                do ii=1,3
                   do jj=1,3
                    virep3b2(jj,ii)=virep3b2(jj,ii)
     &                +virep3bt_tot2_recv(jj,ii)
                   end do
                end do

                call mpi_recv(ep3bt_tot3_recv,1,mpi_real8,i,
     &          3*numtasks+3*i+1,mpi_comm_world,stat,ierr)
                ep3b3=ep3b3+ep3bt_tot3_recv

               call mpi_recv(dep3bt_tot3_recv,3*npole,mpi_real8,i,
     &          3*numtasks+3*i+2,mpi_comm_world,stat,ierr)
                do ii=1,npole
                   do jj=1,3
                     dep3b3(jj,ii)=dep3b3(jj,ii)+dep3bt_tot3_recv(jj,ii)
                   end do
                end do

               call mpi_recv(virep3bt_tot3_recv,3*3,mpi_real8,i,
     &          3*numtasks+3*i+3,mpi_comm_world,stat,ierr)
                do ii=1,3
                   do jj=1,3
                    virep3b3(jj,ii)=virep3b3(jj,ii)
     &                +virep3bt_tot3_recv(jj,ii)
                   end do
                end do
                if(i3.eq.10) then
                  print*,"End loop In final recv iter 10",i
                end if
             end do
     ...
     &                      + delf(j,i) + deg(j,i) + dex(j,i)
      ...
      print*,"Total Potential Energy :",energy,"Kcal/mole"
        ...
        call mpi_buffer_detach(buf3b,totsize3b,ierr)
...
c
     ...
     & (efindiff(3)-efindiff(1))/2.0d0/step,analyticg(2)
      ...
      if(taskid.eq.master) then
        deallocate(dep3b2)
        deallocate(dep3b3)
      deallocate(dep3bt_tot2_recv)
      deallocate(dep3bt_tot3_recv)
      end if
      if (taskid.le.remainder-1) then
         deallocate(buf2)
      end if
       deallocate(buf1)
       deallocate(buf3b)
       deallocate(mollst3mod_recv)
       deallocate(nmollst3mod_recv)
