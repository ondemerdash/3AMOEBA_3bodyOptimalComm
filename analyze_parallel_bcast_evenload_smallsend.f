...
c     molecular dynamics routine, dynamic.f
      program analyze_parallel_bcast_evenload_smallsend
      implicit none
c      include 'sizes.i'
      include 'sizes3b.i'
c      include 'atoms.i'
...
c               print*,"After buf2 attach"
     ...
     &           2*offset*(i-1)+2*(j-1)+1,mpi_comm_world,ierr)
                 ...
                 do i=1,remainder
                   call mpi_send(mollst3mod(1:800,(numtasks*offset+i):
     &             (numtasks*offset+i)),800,mpi_integer,i-1,
     &             2*numtasks*offset+2*(i-1),mpi_comm_world,ierr)
                   call mpi_send(nmollst3mod((numtasks*offset+i):
...
c                 moli1=mol3new(k1)
     ...
     &          master,2*offset*taskid+2*(cntr-1),mpi_comm_world,
     &          stat,ierr)
                call mpi_recv(nmollst3mod_recv_small,1,mpi_integer,
     &          master,2*offset*taskid+2*(cntr-1)+1,mpi_comm_world,
     &          stat,ierr)
...
cccc                moli1=mol3new(k1)
                call mpi_recv(mollst3mod_recv_small,800,mpi_integer,
     &          master,2*numtasks*offset+2*taskid,mpi_comm_world,
     &          stat,ierr)
                call mpi_recv(nmollst3mod_recv_small,1,mpi_integer,
     &          master,2*numtasks*offset+2*taskid+1,mpi_comm_world,
     &          stat,ierr)
