...
c     molecular dynamics routine, dynamic.f
      program Test_analyze_parallel_bcast_evenload
      implicit none
      include 'sizes3b.i'
      include 'sizes.i'
      include 'atoms.i'
      ...
      integer, allocatable :: blocklength(:)
      integer, allocatable :: disp(:)
      integer offset,remainder,start
      integer localsum,totalsum,localsum_tot
      integer stat(MPI_STATUS_SIZE)
c      integer, allocatable :: mollst3mod_recv(:)
      integer, allocatable :: mollst3mod_recv(:,:)
      integer, allocatable :: nmollst3mod_recv(:)
c      integer,allocatable :: mol3new(:)
      character*6 mode
      integer*8 s1,s2,totsize1
      integer*8, allocatable :: buf1(:)
      call mpi_init(ierr)
...
c      allocate (mollst3mod_recv(800,nmol))
c      allocate (nmollst3mod_recv(nmol))

c      allocate (mollst3mod_recv(800))
c      allocate (nmollst3mod_recv(1))
...
c         call mpi_pack_size(800*nmol,mpi_integer,mpi_comm_world,
c     &     s1,ierr)
c         call mpi_pack_size(nmol,mpi_integer,mpi_comm_world,
c     &     s2,ierr)
c           totsize1=s1+s2+2*mpi_bsend_overhead
c           allocate(buf1(totsize1))

c         call mpi_buffer_attach(buf1,totsize1,ierr)
          
         if(taskid.eq.master) then
c            call mpi_send(mollst3mod(1:800,1:offset),800*offset,
c     &      mpi_integer,1,1,mpi_comm_world,ierr)
c            call mpi_send(nmollst3mod,offset,mpi_integer,1,2,
c     &      mpi_comm_world,ierr)
           do i=1,numtasks
             call mpi_send(mollst3mod(1:800,((i-1)*offset+1):i*offset),
     &      800*offset,mpi_integer,i-1,2*(i-1),mpi_comm_world,ierr)
            call mpi_send(nmollst3mod(((i-1)*offset+1):i*offset),offset,
...
c            print*,"Nmollst3mod_recv(1)",1,nmollst3mod_recv(1)
           cntr=0

         call mpi_barrier(mpi_comm_world,ierr)
         if(taskid.eq.1) then
           do k1=1,offset
            print*,"nmollst3mod_recv(k1)",k1,nmollst3mod_recv(k1)
             do i=1,nmollst3mod_recv(k1)
c             do i=1,800
              cntr=cntr+1
c              print*,"mollst3mod_recv",k1,mollst3mod_recv(cntr)
              print*,"mollst3mod_recv",k1,mollst3mod_recv(i,k1)
             end do 
           end do 
         end if
...
c          print*,"Nmollst3mod",1,nmollst3mod(1)
c          tottriples=tottriples+ntriples
          do k1=offset+1,2*offset
          print*,"nmollst3mod(k1) bcasted",k1,nmollst3mod(k1)
           do k=1,nmollst3mod(k1)
            print*,"mollst3mod(k,k1) bcasted",k1,mollst3mod(k,k1)
           end do
          end do 
c         end do
