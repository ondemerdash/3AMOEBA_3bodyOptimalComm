...
c     molecular dynamics routine, dynamic.f
      program analyze_parallel_bcast_evenload_bsend
      implicit none
c      include 'sizes.i'
      include 'sizes3b.i'
c      include 'atoms.i'
...
c      include 'inter.i'
      ...
      integer ierr,taskid,numtasks,master2b,master3b
      integer l1,i2
      integer k1,k2
      integer moli1
...
c
c    Set 0th taskid to master2b task
c
      master2b=0
      master3b=1

      if(taskid.eq.master2b) then
      call settime
      call promo
      end if
...
c      call mechanic
      if(taskid.eq.master2b) then
       ...
       call mpi_barrier(mpi_comm_world,ierr)
         ...
         call mpi_bcast(nmol,1,mpi_integer,master2b,
     &   mpi_comm_world,ierr)
          call mpi_bcast(imol,2*maxatm,mpi_integer,master2b,
     &   mpi_comm_world,ierr)
         print*,"After molecule bcast"

      if(taskid.ne.master2b) then
         call mechanic_parallel
      end if

c          call mpi_bcast(ipole,maxatm,mpi_integer,master2b,
c     &   mpi_comm_world,ierr)
c          call mpi_bcast(xaxis,maxatm,mpi_integer,master2b,
c     &   mpi_comm_world,ierr)
c          call mpi_bcast(yaxis,maxatm,mpi_integer,master2b,
c     &   mpi_comm_world,ierr)
c          call mpi_bcast(zaxis,maxatm,mpi_integer,master2b,
c     &   mpi_comm_world,ierr)
c          call mpi_bcast(npole,1,mpi_integer,master2b,
c     &   mpi_comm_world,ierr)
c          call mpi_bcast(polaxe,8*maxatm,mpi_character,master2b,
c     &   mpi_comm_world,ierr)
         print*,"After mpole var bcast"
...
c     totals of these, dep3bt_tot2 and dep3bt_tot3.  
      ...
      else if (taskid.eq.master3b) then
        allocate (dep3b3(3,npole))
      end if
...
c  Build 2- and 3-body neighbor lists
     ...
     &     s2,ierr)
      ...
      print*,"Before 1st buffer attach"
      call mpi_buffer_attach(buf1,totsize1,ierr)
      print*,"After 1st buffer attach"
     ...
     &      2*numtasks+2*(i-1)+1,mpi_comm_world,ierr)
       ...
       allocate (nmollst3mod_recv_small(1))
          call mpi_recv(mollst3mod_recv,800*offset,mpi_integer,master2b,
     &      2*taskid,mpi_comm_world,stat,ierr)
          call mpi_recv(nmollst3mod_recv,offset,mpi_integer,master2b,
     &      2*taskid+1,mpi_comm_world,stat,ierr)

         if(taskid.le.remainder-1) then
          call mpi_recv(mollst3mod_recv_small,800,mpi_integer,master2b,
     &      2*numtasks+2*(taskid),mpi_comm_world,stat,ierr)
          call mpi_recv(nmollst3mod_recv_small,1,mpi_integer,master2b,
...
c
         call mpi_bcast(nmollst,nmol,mpi_integer,master2b,
     &   mpi_comm_world,ierr)
c         print*,"After 1st mpi_bcast"
          call mpi_bcast(mollst,100*nmol,mpi_integer,master2b,
     &   mpi_comm_world,ierr)
c         print*,"After 2nd mpi_bcast"
c         call mpi_bcast(nmollst3mod,nmol,mpi_integer,master2b,
c     &   mpi_comm_world,ierr)
c         print*,"After 3rd mpi_bcast"
c         call mpi_bcast(mollst3mod,800*nmol,mpi_integer,master2b,
c     &   mpi_comm_world,ierr)
...
c      if(i3.eq.1) then   
         call mpi_bcast(rpole,13*maxatm,mpi_real8,master2b,
     &   mpi_comm_world,ierr)
c         call mpi_barrier(mpi_comm_world,ierr)
c      end if
...
c      they are mpi_reduced.
     ...
     &          mpi_sum,master2b,mpi_comm_world,ierr)
...
cccc                moli1=mol3new(k1)
     ...
     &          mpi_sum,master3b,mpi_comm_world,ierr)
...
c
           call mpi_barrier(mpi_comm_world,ierr)          
           if(taskid.eq.master3b) then
             call mpi_send(ep3b3,1,mpi_real8,master2b,1,
     &       mpi_comm_world,ierr)
             call mpi_send(dep3b3,3*npole,mpi_real8,master2b,2,
     &       mpi_comm_world,ierr)
             call mpi_send(virep3b3,3*3,mpi_real8,master2b,3,
     &       mpi_comm_world,ierr)
           end if
           if(taskid.eq.master2b) then
             call mpi_recv(ep3b3,1,mpi_real8,master3b,1,
     &       mpi_comm_world,stat,ierr)
             call mpi_recv(dep3b3,3*npole,mpi_real8,master3b,2,
     &       mpi_comm_world,stat,ierr)
             call mpi_recv(virep3b3,3*3,mpi_real8,master3b,3,
     &       mpi_comm_world,stat,ierr)
           end if
