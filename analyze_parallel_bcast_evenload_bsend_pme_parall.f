...
c     molecular dynamics routine, dynamic.f
      program analyze_parallel_bcast_evenload_bsend_pme_parall
      implicit none
c      include 'sizes.i'
      include 'sizes3b.i'
      include 'sizes.i'
...
c      integer,allocatable :: mol3new(:)
      ...
      real*8, allocatable :: demt(:,:)
      real*8, allocatable :: demt_tot(:,:)
      real*8, allocatable :: demreal(:,:)
      real*8 emt,emt_tot,viremt(3,3),viremt_tot(3,3),viremreal(3,3)
      real*8, allocatable :: devt(:,:)
      real*8, allocatable :: devt_tot(:,:)
      real*8 evt,emreal,evt_tot,virevt(3,3),virevt_tot(3,3),virev(3,3)
      real*8 e,f,term,fterm,cii,dii,qii,ci,dix,diy,diz,qixx,qixy,qixz
      real*8 qiyy,qiyz,qizz,aewald,ev_total
      integer offsetvdw,remaindervdw,offsetem,remainderem,atomind
...
c     totals of these, dep3bt_tot2 and dep3bt_tot3.  
      allocate (demt(3,npole))
c      allocate (demreal(3,npole))
      allocate (devt(3,n))
      allocate (dep3bt(3,npole))
      if(taskid.eq.master) then
        allocate (dep3b2(3,npole))
        allocate (dep3b3(3,npole))
        allocate (demreal(3,npole))
        allocate (dev_total(3,n))
      end if
      allocate (dep3bt_tot2(3,npole))
      allocate (dep3bt_tot3(3,npole))
      allocate (demt_tot(3,npole))
      allocate (devt_tot(3,n))
...
c      if (use_vdw) then
c         if (vdwtyp .eq. 'LENNARD-JONES')  call elj1
c         if (vdwtyp .eq. 'BUCKINGHAM')  call ebuck1
c         if (vdwtyp .eq. 'MM3-HBOND')  call emm3hb1
c         if (vdwtyp .eq. 'BUFFERED-14-7')  call ehal1
c         if (vdwtyp .eq. 'GAUSSIAN')  call egauss1
c      end if
c      print*,"After vdw"
...
c        call empole1c_3b_Perm
c        call empole1c_3b_Perm_PME
        call chkpole
        call rotpole
        call emrecip1_3b_Perm
        print*,"Perm Elec Recip Before Parallel",em
...
c
                ...
                do i=1,3
                   do j=1,3
                      ...
                      virevt_tot(i,j)=0.0d0
                      viremt_tot(i,j)=0.0d0                    
                   end do
                end do
                do i = 1, n
                   do j = 1, 3
...
c                    print*," devt(j,i)", devt(j,i)
                ...
                evt_tot=evt_tot+evt
                  ...
                  call mpi_reduce(evt_tot,ev_total,1,mpi_real8,
     &          mpi_sum,master,mpi_comm_world,ierr)
                  call mpi_reduce(devt_tot,dev_total,n*3,mpi_real8,
     &          mpi_sum,master,mpi_comm_world,ierr)
...
c      if(i3.eq.1) then
c        do i=1,n
c         print*,"Dep i x",i,dep(1,i)
c         print*,"Dep i y",i,dep(2,i)
c         print*,"Dep i z",i,dep(3,i)
c        end do
c        do i=1,n
c         print*,"Dem i x",i,dem(1,i)
c         print*,"Dem i y",i,dem(2,i)
c         print*,"Dem i z",i,dem(3,i)
c        end do
c        do i=1,n
c         print*,"Dev i x",i,dev(1,i)
c         print*,"Dev i y",i,dev(2,i)
c         print*,"Dev i z",i,dev(3,i)
c        end do
c      end if
      print*,"Vdw, Perm Elec Eng, Polarization Eng",ev,em,ep
